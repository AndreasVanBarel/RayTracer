...
public class Point implements Cloneable, Comparable<Point> {
	...
	/**
	 * Calculates elementwise maximum and returns the result in a new point
	 */
	public Point max(Point a){
		return new Point(Utility.max(a.x, x),Utility.max(a.y, y),Utility.max(a.z, z));
	}
	
	/**
	 * Calculates elementwise minimum and returns the result in a new point
	 */
	public Point min(Point a){
		return new Point(Utility.min(a.x, x),Utility.min(a.y, y),Utility.min(a.z, z));
	}
