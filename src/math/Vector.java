...
public class Vector implements Cloneable, Comparable<Vector> {
	...
	/**
	 * Returns the cosine of the angle between this {@link Vector} and the given {@link Vector}.
	 * @param the {@link Vector} in question
	 * @return the cosine of the angle between this {@link Vector} and the given {@link Vector}
	 */
	...
	public double angle(Vector vec){
		return Math.acos(cos(vec));
	}
	
	/** Returns the vector mirrored by this vector
	 * 
	 */
	public Vector mirror(Vector vec){
		return this.scale(2*vec.dot(this)/lengthSquared()).subtract(vec);
	}
	...
	/**gets angle with z-axis, between -pi/2 and pi/2 */
	public double getPhi(){
		return Math.asin(y/length());
	}
	
	/**gets angle in xy-plane (angle with x-axis), between 0 and 2*pi */
	public double getTheta(){
		double theta = Math.atan2(z, x) + Math.PI;
		return theta;
	}
